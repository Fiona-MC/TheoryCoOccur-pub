...
# def simulate_coOccurrence_randenv(samplesize, numtrials, a, b, alpha, tmax = 30, y0 = None, noise = None):
#     '''
#     n is the number of samples to run
#     samplesize is the number of species to sample from the community
#     numtrials is the number of samples to get for co-occurrence dynamics
...
#     tmax is the ending time for the differential equation
#     y0 is chosen at random if not supplied
#     If noise is not provided, standard GLV will run; if provided, birth rates will be perturbed each trial
#     '''
#     nSpecies = a.shape[0]
...
#             #matrix
#             reducedMx = a[np.ix_(spList, spList)]
#             #also subsample birthrates, with random noise added to baseline 
#             reducedb = b[spList] + np.random.uniform(low = -noise, high = noise, size = (len(spList)))
